...
def get_current_market_cap(mint, retries=3):
    ...
    if mint in _market_cap_cache:
        ...
        if now - cached_at < 300:  # 5 min cache for hits AND misses
    ...
    try:
        ...
        if resp.status_code == 429:
            ...
            # Cache the miss so we don't hammer it
...
def get_market_caps_batch(mints):
    ...
    # Only individually retry mints that were truly missing from a successful response.
    # We do NOT retry mints that are already cached as dead (False).
    missing = [
        m for m in mints
